...
unless ARGV[0] and ARGV[0].start_with? '-'
  ...
  if ENV.key? "DOCKER_FORGE_PROXY"
    config << "--proxy=#{ENV['DOCKER_FORGE_PROXY']}"
  end

  # PUPPET_FORGE_CACHE_TTL="SECS"
  if ENV.key? "DOCKER_FORGE_CACHE_TTL"
    config << "--ram-cache-ttl=#{ENV['DOCKER_FORGE_CACHE_TTL']}"
  end

  # PUPPET_FORGE_CACHE_SIZE="NUM"
  if ENV.key? "DOCKER_FORGE_CACHE_SIZE"
    config << "--ram-cache-size=#{ENV['DOCKER_FORGE_CACHE_SIZE']}"
  end

  # PUPPET_FORGE_MODULE_DIR="DIR[:DIR]"
  if ENV.key? "DOCKER_FORGE_MODULE_DIR"
    if ENV["DOCKER_FORGE_MODULE_DIR"].include? ':'
      ENV["DOCKER_FORGE_MODULE_DIR"].split(":").each do |dir| 
        config << "--module-dir=#{dir}"
      end
    else
      config << "--proxy=#{ENV["DOCKER_FORGE_MODULE_DIR"]}"
    end
  else 
